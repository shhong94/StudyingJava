    ...
    public void movieAllData() 
    {
     try {
	    	...
	    	//for(int i=4;i<=6;i++) //================================================================================================
	    	{
	    		...
	    		Document doc=Jsoup.connect("http://book.interpark.com/display/collectlist.do?_method=bestsellerHourNew&bookblockname=b_gnb&booklinkname=%BA%A3%BD%BA%C6%AE%C1%B8&bid1=w_bgnb&bid2=LiveRanking&bid3=main&bid4=001").get();
	    	    Elements link=doc.select(".listItem.singleType a");// css
...
//		    	    	System.out.println("포스터:"+poster.attr("src"));
		    	    	Element title=doc2.selectFirst(".titleWrap p");
    		...
    		for(int i=1;i<=11;i++)
    		{
    		    ...
    		    for(int j=0;j<title.size();j++)
    		    {
					...
					String data = title.get(j).text() + "|" + 
									content.get(j).text().replace("|", " ") + "|" +		// 문장 안에 "|" 없게끔 공백으로 변환시키기
									author.get(j).text() + "|" +
